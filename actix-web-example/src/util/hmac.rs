...
pub fn hmac_sha256(algorithm: hmac::Algorithm,
                   key_value: &[u8],
                   input: &[u8],
                   output: &[u8],
...
mod tests {
    ...
    pub fn test_hmac_sha256() {
        ...
        let key = HEXUPPER.decode(b"000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2E2F303132333435363738393A3B3C3D3E3F").unwrap();
        let output = HEXUPPER.decode(b"8BB9A1DB9806F20DF7F77B82138C7914D174D59E13DC4D0169C9057B133E1D62").unwrap();
        let result = hmac_sha256(hmac::HMAC_SHA256, &key[..], &input[..], &output[..]);
        match result {
            Ok(out) => {
    ...
    fn hmac_tests() {
        ...
        let k = b"000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F";

        let key_bytes = HEXUPPER.decode(&k[..]);
